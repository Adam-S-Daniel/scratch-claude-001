...
This document runs the full test suite for the art market analysis app and demonstrates that all 205 tests pass across every module. The app was built using strict red/green TDD — every test was written before its implementation.

## Module Overview

The test suite is organized into 10 test files mirroring the source modules:
...
| test_data_loader.py | data_loader.py | 45 | Seed files, Met/Smithsonian/NGA APIs, DataLoader orchestration, risk filtering |
| test_source_registry.py | source_registry.py | 53 | Risk levels, source config, registry, agreements, API keys, edge cases |
...
All 205 tests pass. The suite covers:
...
- **45 data loader tests**: Seed file validation, Met API integration (mocked), Smithsonian API integration (mocked), NGA CSV parsing (mocked), DataLoader orchestration, risk-level filtering (9 new tests)
- **53 source registry tests**: RiskLevel enum ordering/parsing (7), SourceConfig creation/allowed/API keys (11), SourceRegistry filtering/agreements/API keys (16), default source risk levels (14), edge cases (5)
